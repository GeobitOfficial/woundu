...
export function MarketplaceCategorySection({
  ...
  return (
    <section
      ...
      id="categorias"
    >
      <details className="group" open={Boolean(selectedCategorySlug)}>
        <summary className="flex cursor-pointer list-none items-center justify-between gap-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-brand-light/60 px-4 py-5 sm:px-6 [&::-webkit-details-marker]:hidden">
          <div>
          ...
          <div className="flex shrink-0 items-center gap-3">
            <span className="hidden sm:inline-flex">
              <Link
                className="items-center gap-0.5 text-sm font-semibold text-brand hover:underline"
                href={allHref}
              >
            ...
            <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm transition group-open:bg-slate-900 group-open:text-white">
              {selectedCategorySlug ? "Ocultar departamentos" : "Ver departamentos"}
              <ChevronDown
                aria-hidden="true"
        ...
        {categories.length === 0 ? (
          ...
          >
            <div className="mb-4 sm:hidden">
              <Link
                className="inline-flex items-center gap-0.5 text-sm font-semibold text-brand hover:underline"
                href={allHref}
              >
                Ver todas las categorías
                <ChevronRight aria-hidden="true" className="h-4 w-4" />
