...
type AccountBuyerOrdersPaginationProps = Readonly<{
  page: number;
  totalPages: number;
}>;

export function AccountBuyerOrdersPagination({
  page,
  totalPages,
}: AccountBuyerOrdersPaginationProps) {
  if (totalPages <= 1) {
    return null;
  }
  ...
  return (
    ...
    >
      ...
      {nextPage ? (
        <Link
          className={paginationLinkClass}
          href={`/cuenta/compras?page=${nextPage}`}
          rel="next"
        >
          Siguiente
