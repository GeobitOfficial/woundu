...
type SellerEarningsLinesPaginationProps = Readonly<{
  filter: SellerEarningsFilter;
  page: number;
  totalPages: number;
}>;

export function SellerEarningsLinesPagination({
  filter,
  page,
  totalPages,
}: SellerEarningsLinesPaginationProps) {
  if (totalPages <= 1) {
    return null;
  }
  ...
  return (
    ...
    >
      ...
      {nextPage ? (
        <Link
          className={paginationLinkClass}
          href={buildSellerEarningsPageHref(filter, nextPage)}
          rel="next"
        >
          Siguiente
